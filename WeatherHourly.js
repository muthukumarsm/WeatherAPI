...
(function () {
    ...
    myConnector.getSchema = function (schemaCallback) {
        ...
        var hourlytableSchema = {
            id: "hourlyweathercols",
            alias: "Hourly Weather",
            columns: hourlyWeathercols
        };


        schemaCallback([hourlytableSchema]);
    };
 ...
 // var csvstring_test = `Mumbai,Maharashtra,18.987807,72.836447,Delhi,Delhi,28.651952,77.231495`;
        ...
        $.each(data, function (index, row) {
            ...
            $.each(row, function (index, colData) {
                ...
                if (index % 4 == 0) {
                    ...
                    var api_key = "95fc48735d13c772b1464b73f6120578";
                    var url = "https://api.openweathermap.org/data/2.5/onecall?lat=" + lat_coord + "&lon=" + long_coord + "&exclude=minutely,current,daily&appid=" + api_key;
                   ...
                   xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            ...
                            for (i = 0; i < feat.length; i++) {
                                var rainObj = feat[i].rain;
                                var snowObj = feat[i].snow;
                                var rain = "", snow = "";
                                if (rainObj != null) {
                                    rain = feat[i].rain["1h"];
                                }
                             ...
                             var cityandstate = getCityAndState(csvstring_test,json.lat,json.lon);  
                                tableData.push({
                                 ...
                                 "city":cityandstate.split(",")[0],
                                    ...
                                    "lat":json.lat,
                                    "lon":json.lon,
                                    "weatherDate": ConvertEpochToDate(feat[i].dt, "Time"),
                                    "temp": (feat[i].temp - 273.15).toFixed(2),
                                    "feelslike": (feat[i].feels_like - 273.15).toFixed(2),
    ...
    $(document).ready(function () {
        $("#submitButton").click(function () {
            tableau.connectionName = "Weather API Hourly weather" // This will be the data source name in Tableau
