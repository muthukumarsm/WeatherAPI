function ConvertEpochToDate(datevalue, timeValue) {
    ...
    if (timeValue == "") {
        ...
        var fullTime = fullDate + " " + hour + ":" + min + ":" + sec;
        return fullTime;
...
function getCityAndState(csvstring_test,latvalue,longvalue) {
    ...
    for (i1=3;i1<=arraybounds;i1=i1+4) {
         if (parseFloat(data1[0][i1]).toFixed(2) == longvalue && parseFloat(data1[0][i1-1]).toFixed(2) == latvalue) {
            cityval=data1[0][i1-3];
            stateval=data1[0][i1-2];
...
(function () {
    ...
    myConnector.getSchema = function (schemaCallback) {
        ...
        var dailyWeathercols = [{
            ...
            id: "tempmorn",
            alias: "Morning Temp (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "tempeve",
            alias: "Evening Temp (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "tempday",
            alias: "Day Temp (C)",
            ...
            id: "feelslikemorn",
            alias: "FeelsLike in Morning (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "feelslikeeve",
            alias: "FeelsLike in Evening (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "feelslikeday",
            alias: "FeelsLike in Day (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "feelslikenight",
            alias: "FeelsLike in Night (C)",
            dataType: tableau.dataTypeEnum.float
        },{
            id: "pressure",
            alias: "pressure (hPa)",
            ...
            id:"uvi",
            alias:"UV Index (Midday)",
            dataType: tableau.dataTypeEnum.float
        },{
            id:"visibility",
            alias:"visibility Average (m)",
            dataType: tableau.dataTypeEnum.float
        }];
        ...
        var dailytableSchema = {
            id: "dailyweathercols",
            alias: "Daily Weather",
            columns: dailyWeathercols
        };
        schemaCallback([dailytableSchema]);
    };
 ...
 //  var csvstring_test = `Mumbai,Maharashtra,18.987807,72.836447,Delhi,Delhi,28.651952,77.231495`;
    
    data = csvobj.toArrays(csvstring_test);

    var arrayIndexBounds = data[0].length - 1;
    
    var city="",state="", lat_coord = "", long_coord = "";
    var count = 0;
        ...
        $.each(data, function (index, row) {
            ...
            $.each(row, function (index, colData) {
                ...
                if (index % 4 == 0) {
                    ...
                    var api_key = "9fa63c401a707fe70c64e28a884a8155";
                    var url = "https://api.openweathermap.org/data/2.5/onecall?lat=" + lat_coord + "&lon=" + long_coord + "&exclude=minutely,current,hourly&appid=" + api_key;
                        ...
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            ...
                            for (i = 0, len = feat.length; i < len; i++) {
                                ...
                                tableData.push({
                                    ...
                                    "lat":json.lat,
                                    "lon":json.lon,
                                    "weatherDate":ConvertEpochToDate(feat[i].dt,""),
                                    "sunrise":ConvertEpochToDate(feat[i].sunrise,"time"),
                                    "sunset":ConvertEpochToDate(feat[i].sunset,"time"),
                                    "tempmorn": (feat[i].temp.morn  - 273.15).toFixed(2),
                                    "tempeve":(feat[i].temp.eve - 273.15).toFixed(2),
                                    "tempday":(feat[i].temp.day - 273.15).toFixed(2),
                                    "tempnight":(feat[i].temp.night - 273.15).toFixed(2),
                                    "tempmin":(feat[i].temp.min - 273.15).toFixed(2),
                                    "tempmax":(feat[i].temp.max - 273.15).toFixed(2),
                                    "feelslikemorn":(feat[i].feels_like.morn - 273.15).toFixed(2),
                                    "feelslikeeve":(feat[i].feels_like.eve - 273.15).toFixed(2),
                                    "feelslikeday":(feat[i].feels_like.day - 273.15).toFixed(2),
                                    "feelslikenight":(feat[i].feels_like.night - 273.15).toFixed(2),
                                    "pressure":feat[i].pressure,
                                    "humidity":feat[i].humidity,
                                    "dewpoint":(feat[i].dew_point - 273.15).toFixed(2),
                                    "windspeed":feat[i].wind_speed,
                                    "winddegree":feat[i].wind_deg,
                                    "weatherid":feat[i].weather[0].id,
                                    "weathermain":feat[i].weather[0].main,
                                    "weatherdescription":feat[i].weather[0].description,
                                    "clouds":feat[i].clouds ,
                                    "rain":feat[i].rain,
                                    "uvi":feat[i].uvi,
                                    "visibility":feat[i].visibility 
                                });
    ...
    $(document).ready(function () {
        $("#submitButton").click(function () {
            tableau.connectionName = "Weather API Daily weather" // This will be the data source name in Tableau
